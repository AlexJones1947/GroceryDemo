package com.app.grocerydemo.db;
...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    private String texConstKey1 = " TEXT, ";
    ...
    public boolean setCart(Map<String, String> map, Integer qty) {
        ...
        if (isInCart(map.get(VARIENT_ID))) {
            ...
            values.put(COLUMN_FAV, map.get(COLUMN_FAV));
    ...
    public void updatePrice(Map<String, String> map, Integer price) {
        db = getWritableDatabase();
        if (isInCart(map.get(VARIENT_ID))) {
            db.execSQL("update " + CART_TABLE + " set " + COLUMN_PRICE + " = '" + price + "' where " + VARIENT_ID + "=" + map.get(VARIENT_ID));
            db.close();
        }else {
            db.close();
        }
    ...
    public boolean updateFav(Map<String, String> map, String fav) {
        db = getWritableDatabase();
        if (isInCart(map.get(VARIENT_ID))) {
            db.execSQL("update " + CART_TABLE + " set " + COLUMN_FAV + " = '" + fav + "' where " + VARIENT_ID + "=" + map.get(VARIENT_ID));
            db.close();
            return false;
        } else {
            db.close();
    ...
    public List<HashMap<String, String>> getCartAll() {
        ...
        for (int i = 0; i < cursor.getCount(); i++) {
            ...
            map.put(COLUMN_UNIT, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_UNIT)));
            ...
            map.put(COLUMN_FAV, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_FAV)));
    ...
    public String getColumnRewards() {
        ...
        Cursor cursor = db.rawQuery(qry, null);
        cursor.moveToFirst();
        String reward = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_REWARDS));
        if (reward != null) {
